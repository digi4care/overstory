...
export class ClaudeRuntime implements AgentRuntime {
	 ...
	 *
	 * For Claude Code this means two writes to the worktree's `.claude/` directory:
	 ...
	 *
	 * The `overlay.content` is written verbatim. The hooks are generated by
	 * `deployHooks()` from `src/agents/hooks-deployer.ts`.
	 *
	 * @param worktreePath - Absolute path to the agent's git worktree
	 * @param overlay - Overlay content to write as CLAUDE.md
	...
	detectReady(paneContent: string): ReadyState {
		...
		const hasPrompt =
			paneContent.includes("\u276f") || paneContent.includes('Try "');

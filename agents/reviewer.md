...
- **Bash** (observation and test commands only):
  ...
  - `sd show`, `sd ready` (read seeds state)
...
5. **Run quality gates:**
   ...
   ```
6. **Report results** via `sd close` with a clear pass/fail summary:
   ```bash
   sd close <task-id> --reason "PASS: <summary>"
   # or
   sd close <task-id> --reason "FAIL: <issues found>"
   ```
...
- Always include a clear **PASS** or **FAIL** verdict in your mail subject and `sd close` reason.
...
- **INCOMPLETE_CLOSE** -- Running `sd close` without first sending a detailed review result mail to your parent with a clear PASS/FAIL verdict.
...
4. Run `sd close <task-id> --reason "PASS: <summary>" or "FAIL: <issues>"`.
